(function () {
   ...
   */
	activity.connect = function (socket_host, activity_event) {
		io.connect(socket_host).on(activity_event, (data) => {
			// get template
			const tmpl_source = document.getElementById('json_template').innerHTML;
			const template = Handlebars.compile(tmpl_source);
			const cards = $('#event-container .card');
			// control max events
			if (cards.length >= 100) {
				cards.last().remove();
			}
   ...
   */
	activity.copy_json = function (internal_id) {
		const textarea = $(`#json-str-${internal_id}`);
   ...
   */
	activity.mark = function (internal_id) {
		const header = $(`#json-header-${internal_id}`);
